module Agents
  class TwitterFollowStreamAgent < Agent
    ...
    description <<-MD
      ...
      Set `follows` to an array of user IDs, indicating the users whose Tweets should be delivered on the stream. Following protected users is not supported.
    ...
    event_description <<-MD
      When in `counts` mode, TwitterFollowStreamAgent events look like:
      ...
      When in `events` mode, TwitterFollowStreamAgent events look like:
    ...
    def validate_options
      unless (options['filters'].present? || options['follows'].present?) &&
    ...
    def self.setup_worker
      Agents::TwitterFollowStreamAgent.active.order(:id).group_by { |agent| agent.twitter_oauth_token }.map do |oauth_token, agents|
        if Agents::TwitterFollowStreamAgent.dependencies_missing?
          STDERR.puts Agents::TwitterFollowStreamAgent.twitter_dependencies_missing
          STDERR.flush
          return false
        end

        filter_to_agent_map = agents.map { |agent| agent.options[:filters] }.flatten.uniq.compact.map(&:strip).inject({}) { |m, f| m[f] = []; m }
        follow_to_agent_map = agents.map { |agent| agent.options[:follows] }.flatten.uniq.compact.map(&:strip).inject({}) { |m, f| m[f] = []; m }
        ...
        agents.each do |agent|
          ...
          agent.options[:follows].flatten.uniq.compact.map(&:strip).each do |follow|
            follow_to_agent_map[follow] << agent
          end
        end
        ...
        Worker.new(id: agents.first.worker_id(config_hash),
                   config: {filter_to_agent_map: filter_to_agent_map, follow_to_agent_map: follow_to_agent_map},
                   agent: agents.first)
      end
    end
    ...
    class Worker < LongRunnable::Worker
      ...
      def setup
        ...
        @follow_to_agent_map = @config[:follow_to_agent_map]
      end
      ...
      def run
        ...
        EventMachine.run do
          ...
          stream!(@filter_to_agent_map.keys, @follow_to_agent_map.keys, @agent) do |status|
      ...
      private
      def stream!(filters, follows, agent, &block)
        filters = filters.map(&:downcase).uniq
        follows = follows.map(&:downcase).uniq

        path = "/1.1/statuses/#{(filters && filters.length > 0) ? 'filter' : 'sample'}.json#{"?follow=#{follows.map {|f| CGI::escape(f) }.join(",")}" if follows && follows.length > 0}#{"&track=#{filters.map {|f| CGI::escape(f) }.join(",")}" if filters && filters.length > 0}"

        STDERR.puts " --> Twitter path: #{path} at #{Time.now} <--"
